vim.api.nvim_create_user_command("SkipConcealedTextRight", function()
  ...
  for _, mark in ipairs(marks) do
    ...
    if details.conceal ~= nil then
      possible_next_col = details.end_col
...
vim.api.nvim_create_user_command("SkipConcealedTextLeft", function()
  ...
  for _, mark in ipairs(marks) do
    ...
    if details.conceal ~= nil then
      possible_prev_col = start_conceal_col - 1
      if possible_prev_col < prev_col then
        prev_col = possible_prev_col
      end
    end
  end
  vim.api.nvim_win_set_cursor(window_id, { row, math.max(prev_col, 0) })
